...
def _agg(values: list[float]) -> dict:
    ...
    if not values:
        return {}
    return {"min": round(min(values), 4), "media": round(statistics.mean(values), 4), "n": len(values)}


def main() -> None:
    cfg = yaml.safe_load(CONFIG.read_text(encoding="utf-8")) or {}
    status = {"updated": now_iso(), "energy_saved": False, "mobile_saved": False, "notes": []}
    ...
    try:
        ...
        if energy.get("offers"):
            ...
            print(f"  totale: {len(energy['offers'])} offerte")
        else:
